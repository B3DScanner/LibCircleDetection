...
Zikai::sharpTurn Zikai::rejectSharpTurn(
	...
	for (int ii = 0; ii < no_seg_grps; ii++)//no_seg_grps
	{
		...
		const auto&  present_seg_grp = segList[ii];
		...
		for (int seg_no = 0; seg_no < no_of_seg - 1; seg_no++)
		{
			...
			{
				// check again if it is ZIKAI_TRUE sharp turns
	...
	{
		const auto& current_seg = segList[ii];
		const auto& current_edge = edgeList[ii];
