...
double Zikai::PerpendicularDistance(const cv::Point& pt, const cv::Point& lineStart, const cv::Point& lineEnd)
{
...
void Zikai::RamerDouglasPeucker(const std::vector<cv::Point>& pointList, double epsilon, std::vector<cv::Point>& out)// const std::vector<cv::Point> &pointList
{
	if (pointList.size() < 2)
