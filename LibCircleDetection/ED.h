...
namespace Zikai {
	...
	public://ED 函数重载，输入参数类型不同
		ED(cv::Mat _srcImage, GradientOperator _op = PREWITT_OPERATOR, int _gradThresh = 20, int _anchorThresh = 0, int _scanInterval = 1, int _minPathLen = 10, double _sigma = 1.0, bool _sumFlag = ZIKAI_TRUE);
		ED(const ED& cpyObj);
		ED(short* gradImg, uchar* dirImg, int _width, int _height, int _gradThresh, int _anchorThresh, int _scanInterval = 1, int _minPathLen = 10, bool selectStableAnchors = ZIKAI_TRUE);
		ED(EDColor& cpyObj);
		ED();//上面都是构造函数
